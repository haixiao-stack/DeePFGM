...
#  data_i=np.reshape(data_i,(rows,columns))
 data_i = np.loadtxt("../../"+datatype+"/data-2.0/unit"+str_i+"_h01.dat", dtype=np.float64)
 x_i=data_i[:,0:4]
 y_i=data_i[:,5:19]
 x=np.vstack([x,x_i])
 y=np.vstack([y,y_i])
...
x = np.c_[x, new_col]
np.save('../../'+datatype+'/raw_data/Xdata_all_8D',x)
np.save('../../'+datatype+'/raw_data/Ydata_all_8D',y)